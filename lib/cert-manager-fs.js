...
class CertManagerFs extends EventEmitter {
    constructor(root) {
        ...
        this.caKeyLength = 2048
        this.serverKeyLength = 2048
    ...
    getCaKeysAndCert(domain, port) {
        const name = `${domain}:${port}`
        ...
        caItem = generateCaKeysAndCert({keyLength: this.caKeyLength})
    ...
    getServerKeysAndCert(domain, port) {
        const name = `${domain}:${port}`
        ...
        const caKeysAndCert = this.getCaKeysAndCert(domain, port)

        serverItem = generateServerKeysAndCert({domain, caKeysAndCert, keyLength: this.serverKeyLength})
...
exports.CertManagerFs = CertManagerFs
exports.CachingCertManagerFs = CachingCertManagerFs
