...
class Proxy extends EventsEmitter {
    constructor(options) {
        const {
            transports = { 'http:': http, 'https:': https },
    ...
    getDirectConnection(hostname, port, callback) {
        const host = hostname
    ...
    getProxiedConnection(hostname, port, callback) {
        ...
        next((err, server) => {
            ...
            this.getDirectConnection(undefined, server.address().port, callback)
        })
    }
    ...
    onRequestHandler(req, res) {
        ...
        next((request, requestPipelineStream) => {
            ...
            connection.once('upgrade', (connectionResponse) => {
                ...
                this.emit('upgrade', responseOptions)
    ...
    onConnectHandler(req, clientSocket, head) {
        ...
        if (connectOptions.filter === 'passthrough') {
            connectionFunc = this.getDirectConnection.bind(this, connect.hostname, connect.port)
        }
        else
        if (connectOptions.filter === 'pipeline') {
            connectionFunc = this.getProxiedConnection.bind(this, connect.hostname, connect.port)
        }
        else {
